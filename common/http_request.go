...
func httpRequestByCookie(httpUrl, method, sParam string, mHeader map[string]string, setCookie []*http.Cookie) (string, int, []*http.Cookie) {
	...
	if er != nil {
		...
		logs.Error("http request error->", httpUrl, er.Error())
	...
	if httpStart == true {
		...
		if err != nil {
			logs.Error("连接失败", httpUrl, err.Error())
			//两次连接都失败了，需要返回一个空
			return "", statusCode, setCookie
		} else {
			defer resp.Body.Close()
...
func DownFile(sUrl, filepath, fileName string) (string, bool) {
	...
	if er != nil {
		logs.Error("连接请求失败 error->", sUrl, er.Error())
		return "", false
	}
	...
	if uErr != nil {
		logs.Error("url地址无法解析", sUrl, uErr.Error())
		return "", false
	}
	...
	if err != nil {
		logs.Error("读取远程文件失败->", sUrl, err.Error())
	...
	if resp != nil && resp.Body != nil {
		...
		if resp.StatusCode != 200 {
			logs.Error("网络状态异常", sUrl, resp.StatusCode)
			return allPathName, false
		}
		...
		defer out.Close()
		if er != nil {
			logs.Error("创建文件失败", allPathName, er.Error())
			return allPathName, res
		}
		...
		//如果读取不到文件，这里直接返回失败
		if len(body) < 1 {
			logs.Error("获取文件大小失败", sUrl, resp.ContentLength, len(body))
			//删除空文件
		...
		if rSize != int64(aw) {
			logs.Error("文件下载不完整", sUrl, rSize, aw)
			//删除空文件
			os.RemoveAll(allPathName)
			return allPathName, false
		}
		...
		if err != nil {
			logs.Error("写入文件失败", sUrl, err.Error())
			return allPathName, false
		}
		if aErr != nil {
			logs.Error("保存本地文件失败", sUrl, err.Error())
			return allPathName, false
		}
		if resp.ContentLength > 0 && resp.ContentLength != aw {
			logs.Error("文件下载不完整", sUrl, err.Error())
			//删除空文件
			os.RemoveAll(allPathName)
			return allPathName, false
		}
