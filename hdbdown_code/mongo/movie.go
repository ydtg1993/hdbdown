...
func GetMovieDataByNumber(dbName string, number string) (err error, mv *MovieBase) {
	...
	err = DBClient.Collection(dbName).FindOne(context.TODO(), bson.M{"uid": number}, findOptions).Decode(&mv)
...
func Count(dbName string, lastTime string) (err error, num int64) {
	...
	num, err = DBClient.Collection(dbName).CountDocuments(context.TODO(), filter)
	if err != nil {
		return
	}
	return
...
func Find(dbName string, lastTime string, currentPage int64, limit int64) (err error, list []*MovieBase) {
	...
	filter := bson.D{{"ctime", bson.D{{"$gt", lastTime}}}}
	cur, err := DBClient.Collection(dbName).Find(context.TODO(), filter, findOptions)
	if err != nil {
		return
	}
