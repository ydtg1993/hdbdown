...
func ImgIsLoad(filename string) bool {
	...
	allPath := config.Spe.Downpath + "/" + filename

	//判断文件是否存在，不存在的文件需要下载
	chk := tools.IsExist(allPath)
	if chk == false {
		return true
	}

	//文件大小小于1也加入队列
	size, _ := tools.GetFileSize(allPath)
	if size < 1 {
		return true
	}
...
func formatUrl(sUrl string) (bool, string) {
	...
	if strings.Count(sUrl, "http") >= 2 {
		...
		if strings.LastIndex(sUrl, "http://") > 1 || strings.LastIndex(sUrl, "https://") > 1 {
			sUrl = strings.ReplaceAll(sUrl, config.Spe.Downdomain, "")
		}
	}
	//不包含http头的，补上
	if strings.Count(sUrl, "http") <= 0 {
		sUrl = config.Spe.Downdomain + sUrl
	}
	return true, sUrl
}
