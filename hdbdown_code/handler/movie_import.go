...
func (reShip *RelationshipManager) Update(mid int, cid int) (err error) {
	...
	for _, aid := range reShip.ActorID {
		...
		rest := orm.Eloquent.Where("mid = ? and aid = ?", mid, aid).FirstOrCreate(&maa)
		if rest.Error != nil {
			return
		}
		...
		if err = orm.Eloquent.Where("aid = ? and cid = ?", aid, cid).FirstOrCreate(&maca).Error; err != nil {
			return
		}
	}
	...
	for _, id := range reShip.CompaniesID {
		...
		if err = orm.Eloquent.Where("mid = ? and film_companies_id = ?", mid, id).FirstOrCreate(&mcp).Error; err != nil {
			return
		}
		...
		if err = orm.Eloquent.Where("cid = ? and film_companies_id = ?", cid, id).FirstOrCreate(&mfca).Error; err != nil {
			return
		}
	}
	...
	for _, id := range reShip.SeriesID {
		...
		ms.Mid = mid
		ms.SeriesId = id
		if err = orm.Eloquent.Where("mid = ? and series_id = ?", mid, id).FirstOrCreate(&ms).Error; err != nil {
			return
		}
		...
		msca.Cid = cid
		msca.SeriesId = id
		if err = orm.Eloquent.Where("cid = ? and series_id = ?", cid, id).FirstOrCreate(&msca).Error; err != nil {
			return
		}
	}
	...
	for _, id := range reShip.LabelID {
		...
		if err = orm.Eloquent.Where("mid = ? and cid = ?", mid, id).FirstOrCreate(&ml).Error; err != nil {
			return
		}
		...
		mlca.Cid = cid
		mlca.Lid = id
		if err = orm.Eloquent.Where("cid = ? and lid = ?", cid, id).FirstOrCreate(&mlca).Error; err != nil {
			return
		}
	...
	for _, id := range reShip.DirectorID {
		...
		if err = orm.Eloquent.Where("mid = ? and did = ?", mid, id).FirstOrCreate(&ml).Error; err != nil {
			return
		}
	}
...
func ProcessMovieData() {
	...
	for i := 0; i < int(pageAll); i++ {
		...
		for _, v := range res {
			...
			if err != nil {
				logs.Error("ProcessMovieData 创建关系失败:", err.Error())
				continue
			}
		}
...
func createMovieWithRelationship(temporaryData *models.TemporaryMovie) (err error) {
	...
	if mongoData.BigCover == "" && mongoData.SmallCover == "" {
		err = temporaryData.IncompleteData()
		if err != nil {
			logs.Error(temporaryData.Number, "movie 数据修改失败")
		}
		logs.Error(temporaryData.Number, "movie 影片数据二次确认不完整:", temporaryData.Id)
		return
	}
...
func getDirectorId(mongoData *mongo.MovieBase) (err error, ids []int) {
	...
	directID, err := rd.GetCashWithHash(models.DirectorNameWithIDHash, name, func() (id string, err error) {
		...
		if err := orm.Eloquent.Where("name = ?", mongoData.Direct).FirstOrCreate(&mc).Error; err != nil {
			return "", err
		}
...
func getActorId(mongoData *mongo.MovieBase) (err error, ids []int) {
	...
	for _, v := range actor {
		...
		actorId, err := rd.GetCashWithHash(models.ActorNameWithIDHash, v["name"], func() (string, error) {
			...
			movieActor.Oid = 0
			if err := orm.Eloquent.Where("name = ?", v["name"]).FirstOrCreate(&movieActor).Error; err != nil {
				return "", err
			}
...
func getLabelId(mongoData *mongo.MovieBase) (err error, ids []int) {
	...
	for _, labels := range videoSort {
		...
		labelId, err := rd.GetCashWithHash(models.LabelNameWithIDHash, labels, func() (id string, err error) {
			...
			if err := orm.Eloquent.Where("name = ?", labels).FirstOrCreate(&movieLabels).Error; err != nil {
				return "", err
			}
...
func getFilmCompaniesId(mongoData *mongo.MovieBase) (err error, ids []int) {
	...
	filmID, err := rd.GetCashWithHash(models.FilmNameWithIDHash, name, func() (id string, err error) {
		...
		if err := orm.Eloquent.Where("name = ?", mongoData.Producer).FirstOrCreate(&mc).Error; err != nil {
			return "", err
		}
...
func getSeriesId(mongoData *mongo.MovieBase) (err error, ids []int) {
	if len(mongoData.Series) == 0 {
		return nil, nil
	}
	for _, name := range mongoData.Series {
		seriesID, err := rd.GetCashWithHash(models.SeriesNameWithIDHash, name, func() (id string, err error) {
			...
			if err := orm.Eloquent.Where("name = ?", mongoData.Series).FirstOrCreate(&ms).Error; err != nil {
				return "", err
			}
...
func createMovie(mongoData *mongo.MovieBase, relShip RelationshipManager) (Mid int, err error) {
	...
	if err = orm.Eloquent.Where("mid = ? and cid = ?", Mid, movieCategory.Id).FirstOrCreate(&mca).Error; err != nil {
		return
	}
